...
function deltPathCwd(program, object) {
  for (var key in object) {
    var finalPath = isWin ? _path3.default.resolve(program.cwd, object[key]).split(_path3.default.sep).join("/") : _path3.default.resolve(program.cwd, object[key]);
...
function getWebpackCommonConfig(program) {
  var packagePath = _path3.default.join(program.cwd, 'package.json');
  packagePath = isWin() ? packagePath.split(_path3.default.sep).join("/") : packagePath;
  ...
  if (packageConfig.theme && typeof packageConfig.theme === 'string') {
    ...
    // relative path
    if (cfgPath.charAt(0) === '.') {
      cfgPath = _path3.default.resolve(program.cwd, cfgPath);
    }
  ...
  return {
    output: {
      path: isWin() ? _path3.default.join(program.cwd, './dest/').split(_path3.default.sep).join("/") : _path3.default.join(program.cwd, './dest/'),
      filename: jsFileName
    },
    resolve: {
    ...
    //prepend it with cwd
    context: isWin() ? program.cwd.split(_path3.default.sep).join('/') : program.cwd,
