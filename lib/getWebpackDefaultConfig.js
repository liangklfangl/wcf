...
function getWebpackCommonConfig(program) {
  ...
  var packagePath = (0, _path.join)(program.cwd, 'package.json');
  ...
  if (packageConfig.theme && typeof packageConfig.theme === 'string') {
    ...
    // relative path
    if (cfgPath.charAt(0) === '.') {
      cfgPath = (0, _path.resolve)(program.cwd, cfgPath);
    }
  ...
  return {
    output: {
      path: (0, _path.join)(program.cwd, './dest/'),
      filename: jsFileName
    },
    ...
    new _webpack2.default.optimize.CommonsChunkPlugin({
      ...
      minChunks: 2,
      filename: commonName
    }), new _webpack2.default.optimize.MergeDuplicateChunksPlugin(),
    //merge them while duplicating
    new _webpack2.default.optimizeRemoveEmptyChunksPlugin()
    //remove empty chunk
    ]
