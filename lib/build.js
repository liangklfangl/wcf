...
export default function build(program, callback) {
  ...
  if (!program.dev) {
    ...
    defaultWebpackConfig.plugins = [...defaultWebpackConfig.plugins, new webpack.optimize.UglifyJsPlugin({
      beautify: false,
      sourceMap: true,
    ...
    if (process.env.NODE_ENV) {
      defaultWebpackConfig.plugins = [...defaultWebpackConfig.plugins, new webpack.DefinePlugin({
  ...
  if (program.vendor) {
    defaultWebpackConfig.plugins.push(new webpack.optimize.CommonsChunkPlugin({
      name: "common",
      minChunks: 2,
      filename: commonName
    }));
