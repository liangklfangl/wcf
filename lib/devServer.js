...
function bundleWDevServer(defaultWebpackConfig, program) {
  ...
  if (devServerOpt.contentBase === undefined) {
    if (devServerOpt["contentBase"]) {
      ...
      var selfDefinedContentBase = path.dirname(path.resolve(process.cwd(), program.htmlTemplate));
      try {
  ...
  console.log("打包的devServer配置为:", util.inspect(devServerOpt, { showHidden: true, depth: 3 }));
