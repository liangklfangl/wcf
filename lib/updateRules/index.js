...
var cssModuleEnabledConfiguration = [{
  test: /\.scss$/i,
  use: [{
    loader: require.resolve("style-loader"),
    options: {}
...
function updateRules(wpOpt, isDev, disableCSSModules) {
  //如果开启了css modules，那么添加进去
  if (!disableCSSModules) {
    delete wpOpt.disableCSSModules;
    for (var i = 0; i < cssModuleEnabledConfiguration.length; i++) {
  ...
  delete wpOpt.disableCSSModules;
  if (isDev) {
    for (var _i = 0; _i < development.length; _i++) {
      wpOpt.module.rules.push(development[_i]);
    }
  } else {
